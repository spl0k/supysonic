...
def star():
    ...
    app.logger.debug('%s %s %s', id, albumId, artistId)

    def try_star(ent, starred_ent, eid):

        try:
            uid = uuid.UUID(eid)
            app.logger.debug('attempt to star %s with id %s', ent.__name__, uid)
        except:
            return 2, request.error_formatter(0, 'Invalid %s id' % ent.__name__)

        if session.query(starred_ent).get((request.user.id, uid)):
            return 2, request.error_formatter(0, '%s already starred' % ent.__name__)

        e = session.query(ent).get(uid)
        if e:
            app.logger.debug('%s found, starring it', ent.__name__)
            app.logger.debug('Starring ' + ent.__name__ + ' : ' + str(uid))
...
def unstar():
    ...
    def try_unstar(ent, eid):
        ...
        session.query(ent).filter(ent.user_id == request.user.id).filter(ent.starred_id == uid).delete()
        return None
...
def rate():
    ...
    if rating == 0:
        session.query(RatingTrack).filter(RatingTrack.user_id == request.user.id).filter(RatingTrack.rated_id == uid).delete()
        session.query(RatingFolder).filter(RatingFolder.user_id == request.user.id).filter(RatingFolder.rated_id == uid).delete()
    else:
        rated = session.query(Track).get(uid)
        rating_ent = RatingTrack
        if not rated:
            rated = session.query(Folder).get(uid)
