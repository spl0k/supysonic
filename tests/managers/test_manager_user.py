...
class UserManagerTestCase(unittest.TestCase):
    ...
    def create_data(self):
        # Create some users
        alice = self._manager.add("alice", "ALICE", admin=True)
        self.assertIsInstance(alice, db.User)
        self.assertTrue(alice.admin)

        bob = self._manager.add("bob", "BOB")
        self.assertIsInstance(bob, db.User)
        self.assertFalse(bob.admin)

        self.assertIsInstance(self._manager.add("charlie", "CHARLIE"), db.User)
    ...
    def test_legacy_password_hash(self):
        func = self._manager._UserManager__legacy_password_hash
        self.assertEqual(
            func("password", "salt"),
    ...
    def test_separator_in_salt(self):
        ...
        legacy_hash = self._manager._UserManager__legacy_password_hash
        compare = self._manager._UserManager__compare_password
    ...
    def test_auth_upgrades_legacy_hash(self):
        ...
        self.assertIsNone(self._manager.try_auth("user", "wrong"))
        self.assertEqual(db.User.get(name="user").password, stored)

        # Good password: authenticated and hash upgraded
        self.assertEqual(self._manager.try_auth("user", "password"), user)
    ...
    def test_get_user(self):
        ...
        for name in ["alice", "bob", "charlie"]:
            ...
            self.assertEqual(self._manager.get(user.id), user)

        # Get with invalid UUID
        self.assertRaises(ValueError, self._manager.get, "invalid-uuid")
        self.assertRaises(TypeError, self._manager.get, 0xFEE1BAD)

        # Non-existent user
        self.assertRaises(db.User.DoesNotExist, self._manager.get, uuid.uuid4())
    ...
    def test_add_user(self):
        ...
        # Create duplicate
        self.assertRaises(ValueError, self._manager.add, "alice", "Alic3", admin=True)
    ...
    def test_delete_user(self):
        ...
        self.assertRaises(TypeError, self._manager.delete, 0xFEE1B4D)
        self.assertEqual(db.User.select().count(), 3)

        # Delete non-existent user
        self.assertRaises(db.User.DoesNotExist, self._manager.delete, uuid.uuid4())
        self.assertEqual(db.User.select().count(), 3)
    ...
    def test_delete_by_name(self):
        ...
        # Delete non-existent user
        self.assertRaises(db.User.DoesNotExist, self._manager.delete_by_name, "null")
    ...
    def test_try_auth(self):
        ...
        for name in ["alice", "bob", "charlie"]:
            user = db.User.get(name=name)
            authed = self._manager.try_auth(name, name.upper())
            self.assertEqual(authed, user)

        # Wrong password
        self.assertIsNone(self._manager.try_auth("alice", "bad"))
        self.assertIsNone(self._manager.try_auth("alice", "alice"))

        # Non-existent user
        self.assertIsNone(self._manager.try_auth("null", "null"))
    ...
    def test_change_password(self):
        ...
        for name in ["alice", "bob", "charlie"]:
            user = db.User.get(name=name)
            # Good password
            self._manager.change_password(user.id, name.upper(), "newpass")
            self.assertEqual(self._manager.try_auth(name, "newpass"), user)
            # Old password
            self.assertEqual(self._manager.try_auth(name, name.upper()), None)
            # Wrong password
            self.assertRaises(
                ValueError, self._manager.change_password, user.id, "badpass", "newpass"
            )
    ...
    def test_change_password2(self):
        ...
        self.assertRaises(
            TypeError, self._manager.change_password2, uuid.uuid4(), "pass"
        )

        # With existing users
        for name in ["alice", "bob", "charlie"]:
            self._manager.change_password2(name, "newpass")
            user = db.User.get(name=name)
            self.assertEqual(self._manager.try_auth(name, "newpass"), user)
            self.assertEqual(self._manager.try_auth(name, name.upper()), None)

            # test passing the user directly
            self._manager.change_password2(user, "NEWPASS")
            self.assertEqual(self._manager.try_auth(name, "NEWPASS"), user)

        # Non-existent user
        self.assertRaises(
            db.User.DoesNotExist, self._manager.change_password2, "null", "newpass"
        )
